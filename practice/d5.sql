...
select dept_name, count(*) as No_of_Employees from Employees2 group by dept_name;

select count(distinct mgr_no) as No_of_Managers from Employees2;

select max(salary) as Highest, min(salary) as Lowest, max(salary) - min(salary) as Difference from Employees2;

select mgr_no, min(salary) as Min_salary from Employees2 where mgr_no is not NULL group by mgr_no order by Min_salary desc;

select round(max(salary)) as Maximum, round(min(salary)) as Minimum, round(sum(salary)) as Sum, round(avg(salary)) as Average from Employees2;

select dept_name, round(max(salary)) as Maximum, round(min(salary)) as Minimum, round(sum(salary)) as Sum, round(avg(salary)) as Average from Employees2 group by dept_name;

select emp_name, nvl(comm, 0) as commission from emp;
